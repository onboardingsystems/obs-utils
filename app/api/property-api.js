...
var PropertyAPI = {
  ...
  getLeaseForms: function(propertyID, libraryID, callBack) {
    var promise = apiV2({
      method: "get",
      url: "/api/v2/properties/" + propertyID + "/lease_forms"
    });
    promise.then((response) => {
      var promises = response.data.lease_forms.map((join) => {
        return apiV2({
          method: "get",
          url: "/api/v2/libraries/" + libraryID + "/lease_forms/" + join.id
        }).then((response) => {
          return _.filter(response.data.templates, {kind: "lease_agreement"}).map((a) => {
            return {
